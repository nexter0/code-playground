...
m: List[str] = [n_bin[2], n_bin[4], n_bin[5], n_bin[6], n_bin[8], n_bin[9], n_bin[10], n_bin[11]]
m_int: List[int] = [int(x) for x in m]
c_old: List[str] = [n_bin[0], n_bin[1], n_bin[3], n_bin[7]]
c_old: List[int] = [int(x) for x in c_old]
...
c0: int = m_int[0] ^ m_int[1] ^ m_int[3] ^ m_int[4] ^ m_int[6]
c1: int = m_int[0] ^ m_int[2] ^ m_int[3] ^ m_int[5] ^ m_int[6]
c2: int = m_int[1] ^ m_int[2] ^ m_int[3] ^ m_int[7]
c3: int = m_int[4] ^ m_int[5] ^ m_int[6] ^ m_int[7]
...
error_syndrome: str = f'{c3 ^ c_old[3]}{c2 ^ c_old[2]}{c1 ^ c_old[1]}{c0 ^ c_old[0]}'
error_syndrome_decimal: int = int(error_syndrome, 2)
...
if error_syndrome_decimal > 11:
    ...
    m = m[::-1]
    m_str = ''.join(m)
    m_formatted = m_str[:4] + " " + m_str[4:]
    print(f'Original message was: {m_formatted} ({int(m_str, 2)})')
    ...
    # Error syndrome 'points' at information bit
        ...
        m_str = ''.join(m)
        m_formatted = m_str[:4] + " " + m_str[4:]
        print(f'Received message was {m_formatted} ({int(m_str, 2)}).')
        ...
        m_str = ''.join(m)
        m_formatted = m_str[:4] + " " + m_str[4:]
        print(f'Corrected message is: {m_formatted} ({int(m_str, 2)})')
