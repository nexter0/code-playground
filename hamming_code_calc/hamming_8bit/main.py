...
n_bin_arr = [int(x) for x in n_bin if x.isdigit()]
m = n_bin_arr[::-1]
print(f'Your number in binary: {n_bin}')

# Calculate check bits
c0 = m[0] ^ m[1] ^ m[3] ^ m[4] ^ m[6]
c1 = m[0] ^ m[2] ^ m[3] ^ m[5] ^ m[6]
c2 = m[1] ^ m[2] ^ m[3] ^ m[7]
c3 = m[4] ^ m[5] ^ m[6] ^ m[7]
...
hamming_formatted = f'{c0}{c1}{m[0]}{c2} {m[1]}{m[2]}{m[3]}{c3} {m[4]}{m[5]}{m[6]}{m[7]}'
hamming_no_space = f'{c0}{c1}{m[0]}{c2}{m[1]}{m[2]}{m[3]}{c3}{m[4]}{m[5]}{m[6]}{m[7]}'
print(f'Hamming code: {hamming_formatted}')
print(f'Hamming code in decimal: {int(hamming_no_space, 2)}')
